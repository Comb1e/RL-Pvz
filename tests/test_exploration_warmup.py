...
@pytest.mark.parametrize(
    "games,epsilon,entropy",
    [(0, 0.1, 1), (1500, 0.01, 10**-0.5), (3000, 0.001, 0.1), (9000, 0.001, 0.1)],
)
def test_schedule_values_and_floor(games, epsilon, entropy):
    cfg = load_config()
    s = exploration_state(cfg, games)
    assert s.epsilon == pytest.approx(epsilon)
    assert s.entropy_factor == pytest.approx(entropy)
