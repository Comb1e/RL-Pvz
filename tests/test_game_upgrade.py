...
def test_compact_recording_metadata_and_input_isolation(cfg, tmp_path):
    ...
    assert native.metadata["experiment"]["engine"]["package_version"] == "1.3.0"
    native.verify()
...
cfg['environment']['cutoff_seconds'] = 1
cfg['training'].update(budget_unit='decisions', device='cpu', total_steps=64, rollout_size=64, batch_size=32, n_envs=1, n_epochs=1, hidden_sizes=[32,32], eval_interval=64)
