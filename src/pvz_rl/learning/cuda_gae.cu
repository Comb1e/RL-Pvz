extern "C" __global__ void gae(const float *reward, const float *value,
                               const float *starts, const long long *durations, const float *last_value,
  ...
  for (int t = steps - 1; t >= 0; --t) {
    ...
    float discount = durations[k] == 0 ? 1.f : powf(gamma, durations[k]);
    float trace = durations[k] == 0 ? 1.f : powf(gamma * lambda, durations[k]);
    float delta = reward[k] + discount * next * mask - value[k];
    last = delta + trace * mask * last;
    advantage[k] = last;
