...
from pvz_rl.learning.periodic import (
    OPTIMIZER_PROTOCOL,
    ActorUpdateState,
    ActorWindow,
    WindowState,
...
from pvz_rl.policy.grouped_policy import ACTION_DISTRIBUTION, GroupedDistribution
...
class TensorPPO:
    ...
    def collect_slot(
        ...
        with env.device_context():
            ...
            if env.live_view is not None and env.live_view.enabled:
                from pvz_rl.presentation.live_view import Activity

                try:
                    env.live_view.drain()
                    env.live_view.session.publish(force=True)
                    env.live_view.set_activity(Activity.UPDATING)
                except Exception as exc:
                    env.live_view.fail(exc)
